...
Shopify.onItemAdded = function(line_item, form) {
  ...
  //alert(JSON.stringify(line_item.id));
  
    //cart = Shopify.getCart();
  	$.ajax({
      type:"GET",
      url:'/cart.js',
      dataType: "json",
      success: function(result){
        ...
        var $html = '<div class="header-ajaxcart">'+result.item_count+' Item'+$multi+' added into cart</div>';
        $.each(result.items, function(i,item){
          var ij = i+1;
          $html += '<div class="row ajaxcartitems">';
              $html += '<div class="col-sm-3">';
                  $html += '<img width="100%" src="'+item.image+'" alt="'+item.title+'">';
              $html += '</div>';
              $html += '<div class="col-sm-5">';
                  $html += '<p class="product-name"><a href="'+item.url+'">'+item.title+'</a></p>';
          	      $html += '<div class="ajx-product-qty"><input class="form-control input-small" type="number" name="updates[]" size="4" id="updates_'+item.id+'" value="'+item.quantity+'" /></div>';
              $html += '</div>';
              $html += '<div class="col-sm-2">';
                  $html += '<div class="cart-collateral"><span class="price">'+formatMoney(item.price)+'</span></div>';
              $html += '</div>';
              $html += '<div class="col-sm-2">';
                  $html += '<a href="/cart/change?line='+ij+'&quantity=0">Remove</a>';
              $html += '</div>';
          $html += '</div>';
        ...
        $('.cartitemcount').text(result.item_count+' Item'+$multi+' in cart');
        $('.ajaxitems').html($html);
        $("#ajaxcart").modal('show');
      }
    })
  	//alert(JSON.stringify(line_item));
